...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (form) {
        form.addEventListener('submit', (e) => {
            ...
            .then(data => {
                if (data.success) {
                    btn.innerText = '✓ E-mail byl úspěšně odeslán!';
                    btn.style.backgroundColor = '#10b981';
                    
                    const successDiv = document.createElement('div');
                    successDiv.className = 'form-message-alert';
                    successDiv.style.cssText = 'background: rgba(16, 185, 129, 0.15); border: 2px solid #10b981; border-radius: 12px; padding: 18px 20px; margin-bottom: 1.5rem; text-align: center; color: #ffffff; box-shadow: 0 4px 20px rgba(16, 185, 129, 0.2);';
                    successDiv.innerHTML = `
                        <div style="font-size: 1.15rem; font-weight: 700; color: #34d399; margin-bottom: 6px; display: flex; align-items: center; justify-content: center; gap: 8px;">
                            <span>✓</span> <span>E-mail byl úspěšně odeslán!</span>
                        </div>
                    ...
                    errorDiv.innerHTML = `
                        <div style="font-size: 1rem; font-weight: 700; color: #f87171; margin-bottom: 4px;">⚠️ Chyba při odesílání e-mailu</div>
                        <p style="margin: 0; font-size: 0.85rem; color: #fecaca;">${data.message || 'Zprávu se nepodařilo odeslat.'}</p>
                    `;
            ...
            .catch(err => {
                ...
                errorDiv.innerHTML = `
                    ...
                    <p style="margin: 0; font-size: 0.85rem; color: #fecaca;">Chyba při komunikaci se serverem. Zkontrolujte prosím připojení.</p>
                `;
                form.prepend(errorDiv);
                btn.innerText = originalText;
            })
