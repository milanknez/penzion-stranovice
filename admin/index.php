...
<body class="flex h-screen w-screen overflow-hidden bg-slate-950 text-slate-100">
    ...
    <script>
        ...
        function openPluginHelpModal(pluginId) {
            ...
            if (plugin.settings_modal) {
                settingsButtonHtml = `
                    <div class="pt-3 border-t border-white/5 flex items-center justify-between">
                        <span class="text-xs text-slate-400">Tento plugin nabízí konfiguraci a návod:</span>
                        <button onclick="${plugin.settings_modal}; closePluginHelpModal();" class="bg-indigo-600 hover:bg-indigo-500 text-white font-bold px-3.5 py-1.5 rounded-xl text-xs transition-all flex items-center gap-1.5">
                            <i class="fa fa-cog"></i> ${plugin.settings_button || 'Nastavení pluginu'}
                        </button>
                    </div>`;
            }
                ...
                <div class="space-y-4">
                    <div class="bg-slate-950 border border-white/10 rounded-xl p-4">
                        ...
                        <div class="text-[11px] text-slate-400 space-y-1 font-mono border-t border-white/5 pt-2">
                            ...
                            <div>Identifikátor: <span class="text-white">${plugin.id}</span></div>
                        </div>
                        ${settingsButtonHtml}
                    </div>
